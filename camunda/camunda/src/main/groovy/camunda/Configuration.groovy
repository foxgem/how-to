...
class Configuration {
  ...
  static def config(String property) {
    ...
    // In case we have a value and know a validator, we use it now
      ...
      keys.addAll(Holders.config.flatten().keySet().collect { it.toString() })
      ...
      value = [:]
      keys.toSet().findAll { it.startsWith("${property}.") && !it.endsWith(']') }.each { prop -> 
        if (exists(prop)) 
  ...
  static Object getConfigObjectProperty(String property) {
    def value = Holders.config.flatten()."$property"
  ...
  static void setConfigObjectProperty(String property, Object value) {
    Holders.config.merge(["$property": value != null ? value : Null])
  }
  ...
  static void clearConfigObjectProperty(String property) {
    Holders.config.merge(["$property": null])  
  }
  ...
  static boolean configObjectContainsKey(String property) {
    Holders.config.flatten().containsKey(property) 
  }
